...
if args.command == "setup":
    ...
    try:
        ...
        try:
            ...
            if e.returncode == 2:
                raise FileNotFoundError(f"Key {key} does not exist") from None
            else:
                raise e
    except KeyError:
        ...
        out = subprocess.check_output(["gpg", "--list-keys"]).decode().strip()
...
"""
    ...
    if added_debs:
        ...
        subprocess.check_output(["reprepro", "-b", REPO_FILES_LOCATION, "export"])
        print(f"Added:\n{s.strip()}")
    else:
